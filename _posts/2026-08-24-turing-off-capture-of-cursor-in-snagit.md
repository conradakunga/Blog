---
layout: post
title: Turing Off Capture Of Cursor In SnagIt
date: 2026-08-24 16:17:33 +0300
categories:
    - SnagIt
    - Tools
---

For over a decade now, my goto for screen capture has always been TechSmith's SnagIt.

By default, when you take a screen capture using SnagIt, it will look like this:

![snagItCapture](../images/2026/08/snagItCapture.png)

Note that the location of the **cursor**.

This you can delete by selecting and deleting.

![SnagItCrop](../images/2026/08/SnagItCrop.png)

However it can be annoying to have to keep doing it.

The solution is in the capture settings:

![snagItCaptureWindow](../images/2026/08/snagItCaptureWindow.png)

Turn off the following setting:

![snagItCropDefault](../images/2026/08/snagItCropDefault.png)

So that it looks like this:

![snagItFinal](../images/2026/08/snagItFinal.png)

And now new captures will no longer have the cursor.

Happy hacking!
