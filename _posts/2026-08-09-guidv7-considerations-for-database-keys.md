...
This, naturally, is an issue should you be generating keys in the database.
...
We will store that in this PostgreSQL database
...
```c#
var connectionString = "host=localhost;username=myuser;password=mypassword;database=things";
...
1. Dapper, for database access and manipulation
2. Npgsql ADO.NET provider
...
Note here that t**he order is not what we are expecting**, due to the issue pointed out earlier about `Guid` generation when the **millisecond timestamps** are the same.
